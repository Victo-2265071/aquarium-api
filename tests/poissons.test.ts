...
describe('poissonsRouter', () => {
  let dbPoisson: IPoisson[] = [];
  ...
  describe(`'GET:${Paths.Poissons.Get}'`, () => {
    ...
    it(
      ...
      async () => {
        ...
        const data = [...DB_Poisson];
        mockify(Poisson).toReturn(data, 'find');
        const res: TRes<{ poisson: IPoisson[] }> = await agent.get(
          Paths.Poissons.Get,
        );
        expect(res.status).toBe(HttpStatusCodes.OK);
        expect(compareUserArrays(res.body.poisson, DB_Poisson)).toBeTruthy();
      },
    );
  });
});
